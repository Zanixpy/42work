...
/*   Updated: 2026/03/14 18:00:59 by omawele          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_threads(t_philo **philos, t_monitor **monitor, int size)
{
	...
	while (++i < size)
	{
		...
		if (pthread_create(&(*philos)[i].tid, NULL, &routine_philosopher,
				(void *)&(*philos)[i]) != 0)
			return (1);
	}
...
int	create_thread_one_philosopher(t_philo **philos, t_monitor **monitor)
{
	...
	(*monitor)->start_time = start_time;
	if (pthread_create(&(*philos)[0].tid, NULL, &routine_only_one_philosopher,
			(void *)&(*philos)[0]) != 0)
		return (1);
	if (pthread_create(&(*monitor)->tid, NULL, &routine_monitor,
			(void *)(*monitor)) != 0)
...
int	main(int argc, char **argv)
{
	...
	{
		if (create_threads(&philos, &monitor, args.size_phi))
			return (clean(forks, philos, monitor, args.size_phi),
				ERRTHREAD);
	}
	return (clean(forks, philos, monitor, args.size_phi), EXIT_SUCCESS);
}
