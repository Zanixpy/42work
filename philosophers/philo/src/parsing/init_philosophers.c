...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/14 18:47:45 by omawele           #+#    #+#             */
/*   Updated: 2026/03/14 18:54:31 by omawele          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		pthread_mutex_t *print_mutex, pthread_mutex_t *stop_mutex)
{
	...
	philos = malloc(args->size_phi * sizeof(t_philo));
	if (!philos)
		return (NULL);
	i = -1;
	while (++i < args->size_phi)
	{
		philos[i].index = i;
		...
		if (i == 0)
			philos[i].right_fork = forks[args->size_phi - 1];
		else
