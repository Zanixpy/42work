...
/*   Updated: 2025/12/06 20:14:56 by omawele          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if ((x == 0 || x == var->map.xsize - 1) && var->map.map[y][x] != '1')
        return (1);
    if (y == 0 || y == var->map.ysize - 1 && var->map.map[y][x] != '1')
        return (1);
...
static void check_wall(t_var *var)
{
    ...
    x = 0;
    y = 0;
    while (var->map.map[0][x] == '1' && var->map.map[0][x] == '1' && x < var->map.xsize)
        x++;
    if (x != var->map.xsize)
        return;
    while (var->map.map[y][0] == '1' && var->map.map[y][x - 1] == '1' && y < var->map.ysize)
        y++;
    if (y != var->map.ysize)
        return;
    var->cmap.wall = 1; 
...
int check_characters(t_var *var)
{
    ...
    y = 1;
    while (y < var->map.ysize - 1)
    {
        x = 1;
        while (x < var->map.xsize - 1)
        {
            if (is_wscep(var, x, y))
                return (1);
            x++;
